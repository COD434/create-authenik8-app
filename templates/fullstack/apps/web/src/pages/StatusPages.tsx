...
function StatusPage({ icon, title, message }: { icon: React.ReactNode; title: string; message: string }) {
  return <main className="status-page"><div className="status-icon">{icon}</div><h1>{title}</h1><p>{message}</p><Link className="button button-primary" to="/dashboard"><ArrowLeft size={17} /> Return to dashboard</Link></main>;
}
