...
describe("generator happy paths", () => {
  ...
  it("writes only the Prisma build approvals required by pnpm and Bun", async () => {
    ...
    try {
      ...
      expect(pnpmConfig).toContain('"find-my-way": "9.7.0"');
      expect(pnpmConfig).toContain('valibot: "1.4.2"');
  ...
  it("generates the connected full-stack workspace without legacy package rewrites", async () => {
    ...
    try {
      ...
      expect(pkg.overrides["find-my-way"]).toBe("9.7.0");
      expect(pkg.overrides.valibot).toBe("1.4.2");
      expect(pkg.overrides["@hono/node-server"]).toBeUndefined();
      ...
      expect(webPkg.dependencies["react-router"]).toBe("8.3.0");
      expect(webPkg.dependencies["react-router-dom"]).toBeUndefined();
